...
####################################################################################################
PVCA <- function(counts, meta, threshold, inter=FALSE, log=FALSE){

  ##counts.center <- t(apply(counts, 1, scale, center=TRUE, scale=FALSE))
  if(log) {counts <- log(counts + 1)}
  counts.center <- (counts - rowMeans(counts))
...
PVCA.svd <- function(counts, meta, threshold, inter=FALSE, log=FALSE){

  ##counts.center <- t(apply(counts, 1, scale, center=TRUE, scale=FALSE))
  if(log) {counts <- log(counts + 1)}
  counts.center <- (counts - rowMeans(counts))
  eigen.counts <- svd(counts.center)
  eigen.mat <- eigen.counts$v
  eigen.val <- eigen.counts$d
  n.eigen <- length(eigen.val)
  eigen.val.sum <- sum(eigen.val)
  percents.pcs <- eigen.val/eigen.val.sum
  meta <- as.data.frame(meta)
...
PlotPVCA <- function(pvca.res, title,
  ...
  p <- p + theme(axis.title.x = element_text(size = axis.title.size, vjust=-0.5))
  p <- p + theme(axis.title.y = element_text(size = axis.title.size, vjust= 1.0))
