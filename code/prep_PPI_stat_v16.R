...
PPI.stat = all.stat %>%
  ...
  filter(procedure_name == "Acoustic Startle and Pre-pulse Inhibition (PPI)") %>%
  mutate(procedure_name=recode(procedure_name, "Acoustic Startle and Pre-pulse Inhibition (PPI)"="PPI")) %>%
...
table(PPI.stat$parameter_name, PPI.stat$procedure_name)

PPI.stat <- PPI.stat[,c("procedure_name","parameter_name","zygosity","allele_symbol","marker_symbol","z_score",
                      "proc_param_name","gene_pheno")]

head(PPI.stat)
dim(PPI.stat)
saveRDS(PPI.stat, file="data/PPI.stat.v16.rds")
