...
CC.data <- allpheno[,c("biological_sample_id","procedure_name","parameter_name","data_point","sex","phenotyping_center","strain_name","proc_short_name",
                        "proc_param_name","proc_param_name_stable_id")]
dim(CC.data)
saveRDS(CC.data, file="data/CC.data.rds")
...
CC.stat <- CC.stat[,c("procedure_name","parameter_name","zygosity","allele_symbol","marker_symbol","z_score",
                      "proc_param_name","gene_pheno")]

head(CC.stat)
dim(CC.stat)
saveRDS(CC.stat, file="data/CC.stat.rds")
